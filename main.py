...
try:
    ...
    #using tkinter to make a graphic interface
except:
...
try:
    ...
    if tkinter.messagebox.askyesno("Error", "MySQL is not installed\nYou need to install MySQL\nWould you like to run MySQL installer?"):
        try:
            ...
            tkinter.messagebox.showerror("Error", "Sorry application N.A.\nKindly download mysql youself.")
        sys.exit(0)
    else:
...
def main(entry, *args, **kwargs):     #first entry with database deatails; getting all details by user by entry widget
    ...
    title= Label(entry, text="Welcome to Store Manage", font=("Stylish",21, "bold"), bg="beige", fg="#EE8262")
    ...
    def func_default(*args, **kwargs):
        ...
        btn_default.config(text="Set deafult values", command=fun_default)
    ...
    def else_load(*args, **kwargs):
        ...
        def create_fun(*args, **kwargs):
            def createe(*args, **kwargs):
                ...
                try:
                    ...
                    tkinter.messagebox.showerror("Error", "Database already exists, enter a proper name!")
                    return
                global uname
                uname= user
                global pwd
            ...
            entry.title("STORE MANAGEMENT-nitish.create_database")
            db_e.delete(0, END)
            db_e.focus()
        ...
        def load_fun(*args, **kwargs):
            ...
            entry.title("STORE MANAGEMENT-nitish.load_database")
            deff.destroy()
            ...
            title= Label(fr, text="Welcome to Store Manager by Nitish", font=("Comic Sans", 22, "bold"), bg="beige", fg="#EE8262")
            title.place(x=30, y=10)
            ...
            def loadd(*args, **kwargs):     #sql query for loading data
                ...
                try:
                    ...
                    return
                try:
                    c.execute(f"CREATE DATABASE {db1_e.get()};")
                    c.execute(f"use {db1_e.get()};")
                    c.execute(query)
                except:
                    tkinter.messagebox.showerror("Error", "Kindly enter proper name")
                    return
                global uname
                uname= uname1_e.get()
                global pwd
...
#ENTRY WINDOW    
entry=Tk()
entry.title("STORE MANAGEMENT-nitish.enter")  #putting title name of window
...
def stock_addition(*args, **kwargs):     #stock addition section to add data in the db
    ...
    class Stock_addition:           #running Stock_addition class when stock_addition button is called
        def __init__(self, root,*args, **kwargs):
            root1.title("STORE MANAGEMENT-nitish.Addition")
            try:
            ...
            def back(*args, **kwargs):
                root1.title("STORE MANAGEMENT-nitish.Billing")
...
def stock_modify(*args, **kwargs):     #stock modification section to modify data in the db
    ...
    class Stock_modify:                  #running Stock_Modify class when stock_modify button is called
        def __init__(self, root,*args, **kwargs):
            root1.title("STORE MANAGEMENT-nitish.Modification")
            self.conn= sql.connect(host="localhost", user=uname, passwd=pwd, database=dbName)
            canvas= Frame(root, width=765, height=600, bg="#88BDBC")
...
def stock_review(*args, **kwargs):     #inside the stock addition section
    class Stock_review:                #running Stock_review class when stock_review button is called
        def __init__(self, root,*args, **kwargs):
            root1.title("STORE MANAGEMENT-nitish.Stock_Review")
            self.conn= sql.connect(host="localhost", user=uname, passwd=pwd, database=dbName)
            ...
            def callback(selection, *args, **kwargs):
                if selection=="By Date":
                    ...
                    def callme(*args, **kwargs):
                        ...
                        data= None
                        if check!=():
                            c.execute(f"select * from {TableName} where date in {check}")
                            data = c.fetchall()
                        else:
                            nd=sel+" "+s.get()
...
def sold_item_review(*args, **kwargs):      #inside the stock review section
    class Sold_item_review:                 #running Sold_item_review class when sold_item_review button is called
        def __init__(self, root,*args, **kwargs):
            root1.title("STORE MANAGEMENT-nitish.Sold_Item_Review")
            self.conn= sql.connect(host="localhost", user=uname, passwd=pwd, database=dbName)
            ...
            def show_bill(*args, **kwargs):
                ...
                if selc!=():
                    ...
                    try:
                        ...
                        return
                    file.insert(-7, "\n")
                    txt.insert(END,"\t\t Store Management\n")
...
================================================\n''')
            ...
            def fun_sortmonth(*args, **kwargs):
                ...
                def callme(*args, **kwargs):
                    ...
                    data= None
                    if check!=():
                        c.execute(f"select * from transactions where date in {check}")
                        data = c.fetchall()
                    else:
                        nd=sel+" "+s.get()
                        self.ll.config(text=f"No Data Found for {nd}")
...
class Billing:
    def __init__(self,root1,left, right, *args, **kwargs):
        ...
        clock()
        headl= Label(self.left, text= "Store Management" ,  font=("arial 30 bold"), bg="#88BDBC", fg="darkblue")
...
root1.title("STORE MANAGEMENT-niish.Billing")
